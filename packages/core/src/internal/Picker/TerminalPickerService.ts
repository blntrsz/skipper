...
const filter = (query: string, options: string[]): string[] => {
  ...
  return options
    .map((opt) => {
      ...
      const score = n.startsWith(query)
        ? 0
        : n.indexOf(query) === -1
          ? Infinity
          : n.indexOf(query) + 1;
    ...
    .sort((a, b) =>
      a.score !== b.score ? a.score - b.score : a.opt.localeCompare(b.opt),
    )
    .map((x) => x.opt);
};
...
export const TerminalPickerService = ServiceMap.make(PickerService, {
  pick: ({ options, message }) =>
    Effect.callback<string, PickerCancelled | PickerError | PickerNoMatch>(
      (resume) => {
        ...
        const render = () => {
          ...
          const start = Math.max(
            0,
            Math.min(idx, Math.max(m.length - MAX_VISIBLE, 0)),
          );
