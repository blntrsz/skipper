...
export async function resolveWorkspaceGitContext(
  cwd: string,
): Promise<WorkspaceGitContext> {
  const branch = await readGitCommand(cwd, [
    "git",
    "rev-parse",
    "--abbrev-ref",
    "HEAD",
  ]);
  const commonDir = await readGitCommand(cwd, [
    "git",
    "rev-parse",
    "--git-common-dir",
  ]);
  const repositoryFromCommonDir = resolveRepositoryFromCommonDir(
    cwd,
    commonDir,
  );
  ...
  const remote = await readGitCommandOptional(cwd, [
    "git",
    "config",
    "--get",
    "remote.origin.url",
  ]);
  const repositoryFromRemote =
    remote !== undefined ? repositoryFromRemoteUrl(remote) : undefined;
  ...
  const topLevel = await readGitCommand(cwd, [
    "git",
    "rev-parse",
    "--show-toplevel",
  ]);
...
async function readGitCommandOptional(
  cwd: string,
