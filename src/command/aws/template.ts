...
const WEBHOOK_TASK_COMMAND = [
  ...
  'runuser -u runner -- env REPO_URL="$REPO_URL" PROMPT="$PROMPT" ECS_AGENT="${ECS_AGENT:-claude}" SKIPPER_WORKER_TYPE="${SKIPPER_WORKER_TYPE:-}" GITHUB_REPO="${GITHUB_REPO:-}" GITHUB_PR_NUMBER="${GITHUB_PR_NUMBER:-}" GITHUB_PR_HEAD_SHA="${GITHUB_PR_HEAD_SHA:-}" GITHUB_PR_BASE_SHA="${GITHUB_PR_BASE_SHA:-}" GITHUB_PR_URL="${GITHUB_PR_URL:-}" GITHUB_TOKEN="${GITHUB_TOKEN:-}" GH_TOKEN="${GH_TOKEN:-${GITHUB_TOKEN:-}}" ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" ANTHROPIC_MODEL="${ANTHROPIC_MODEL:-}" OPENCODE_MODEL="${OPENCODE_MODEL:-amazon-bedrock/${ANTHROPIC_MODEL:-eu.anthropic.claude-sonnet-4-6}}" PATH="/home/runner/.bun/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" bash -lc \"if [ -n \\\"${GITHUB_TOKEN:-}\\\" ] && [ \\\"${REPO_URL#https://github.com/}\\\" != \\\"$REPO_URL\\\" ]; then REPO_URL=\\\"https://x-access-token:${GITHUB_TOKEN:-}@${REPO_URL#https://}\\\"; fi; rm -rf /home/runner/repo; git clone --depth 1 \\\"$REPO_URL\\\" /home/runner/repo; cd /home/runner/repo; if [ \\\"${SKIPPER_WORKER_TYPE:-}\\\" = \\\"code-review\\\" ]; then bun ./src/worker/review/inline-runner.ts; elif [ \\\"${ECS_AGENT}\\\" = \\\"opencode\\\" ]; then opencode run -m \\\"\\$OPENCODE_MODEL\\\" \\\"$PROMPT\\\"; else claude --dangerously-skip-permissions -p \\\"$PROMPT\\\"; fi\"',
].join("; ");
